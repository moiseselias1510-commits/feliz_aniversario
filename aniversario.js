 ...
 *
 *  QUANDO ELA COMEÇA
 *  Só depois de ABRIR O SITE: primeiro a senha, depois a introdução, e a
 *  animação entra quando o conteúdo principal aparece. Ela nunca aparece na
 *  tela de senha nem na introdução. O parâmetro de teste abaixo antecipa
 *  apenas a DATA — para ver a animação você precisa entrar com a senha.
 *
 *  PRÉ-VISUALIZAR FORA DA DATA (sem alterar o funcionamento normal):
 *      index.html?aniversario-data=18-10        -> finge que hoje é 18/10 do ano atual
 *      index.html?aniversario-data=18-10-2027   -> finge 18/10/2027
...
(function () {
  ...
  var CFG = {
    ...
    /* Quando o texto "Feliz X anos de namoro" aparece:
       null  = no FIM da animação, quando o coração se abre (recomendado)
       número = milissegundos após o início (ex.: 300 = logo no começo) */
    atrasoTexto: null,

    /* Quando começar:
       "site" = só depois de ABRIR O SITE (senha + introdução) — recomendado.
       "load" = assim que a página carrega (aparece já na tela de senha — não use). */
    iniciarQuando: "site",

    /* Parâmetros de teste na URL */
    param: "aniversario",
    paramData: "aniversario-data"
  };
  ...
  /* --------------------------------------------------------------------------
     DATA FINGIDA (só para testar fora do dia 18/19 de outubro)
       ?aniversario-data=18-10          -> dia-mês (usa o ano atual)
       ?aniversario-data=18-10-2027     -> dia-mês-ano
       ?aniversario-data=2027-10-18     -> ano-mês-dia
     Sem esse parâmetro nada muda: o script usa a data real do aparelho.
     -------------------------------------------------------------------------- */
  function lerDataFalsa() {
    var v = null;
    try {
      v = new URLSearchParams(window.location.search).get(CFG.paramData);
    } catch (e) { v = null; }

    if (v === null && window.location.hash) {
      var m = window.location.hash.match(new RegExp(CFG.paramData + "=([^&]+)", "i"));
      if (m) v = decodeURIComponent(m[1]);
    }
    if (!v) return null;

    var p = String(v).trim().replace(/[\/.]/g, "-").split("-");
    var n = [];
    for (var i = 0; i < p.length; i++) n.push(parseInt(p[i], 10));
    for (var j = 0; j < n.length; j++) if (!isFinite(n[j])) return null;

    var d, mo, a;
    if (n.length === 2) {
      d = n[0]; mo = n[1]; a = new Date().getFullYear();
    } else if (n.length === 3) {
      if (p[0].length === 4) { a = n[0]; mo = n[1]; d = n[2]; }
      else { d = n[0]; mo = n[1]; a = n[2]; }
    } else {
      return null;
    }

    if (d < 1 || d > 31 || mo < 1 || mo > 12 || a < 1900 || a > 2200) return null;
    return { dia: d, mes: mo, ano: a };
  }

  var DATA_FALSA = lerDataFalsa();
  ...
  function anosAtivos() {
    ...
    /* se veio ?aniversario-data=..., usa a data fingida em vez da real */
    if (DATA_FALSA) {
      dia = DATA_FALSA.dia;
      mes = DATA_FALSA.mes;
      ano = DATA_FALSA.ano;
    ...
    if (mes === CFG.mesFesta && CFG.diasFesta.indexOf(dia) !== -1) {
      anos = ano - CFG.anoInicio;
    }

    if (param === true) {
      if (anos < 1) anos = Math.max(1, ano - CFG.anoInicio);
      return anos;
    }
  ...
  var ANOS = anosAtivos();
  var FORCADO = lerParam() === true || window.M1A_FORCAR === true || !!DATA_FALSA;
  ...
  /* ==========================================================================
     Cartões de exemplo: usados só enquanto a pasta "mais1ano" está vazia,
     para você já ver o movimento. Somem sozinhos assim que houver foto real.
     ========================================================================== */
  function gerarPlaceholders(qtd) {
    var paletas = [
      ["#ff9a9e", "#fecfef"], ["#a18cd1", "#fbc2eb"], ["#f6d365", "#fda085"],
      ["#84fab0", "#8fd3f4"], ["#a1c4fd", "#c2e9fb"], ["#fbc2eb", "#a6c1ee"]
    ];
    var lista = [];

    for (var i = 0; i < qtd; i++) {
      var c = paletas[i % paletas.length];
      var svg =
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400">' +
          ...
          '<text x="200" y="176" font-size="170" text-anchor="middle" ' +
            'dominant-baseline="central" fill="rgba(255,255,255,.92)">\u2665</text>' +
          '<text x="200" y="332" font-size="32" text-anchor="middle" ' +
            'fill="rgba(255,255,255,.92)" ' +
            'font-family="Segoe UI, Arial, sans-serif">foto ' + (i + 1) + '</text>' +
        '</svg>';
      lista.push("data:image/svg+xml;charset=UTF-8," + encodeURIComponent(svg));
    }
    return lista;
  }
  ...
  function descobrirFotos() {
    ...
    return encontrarEsquema().then(function (par) {
      ...
      /* Pasta vazia (ou ainda sem fotos)? Roda com cartões de exemplo para
         você já ver a animação. Some sozinho quando existir foto de verdade. */
      if (lista && lista.length) return lista;
      console.info('[aniversario] Nenhuma foto encontrada em "' + CFG.pastaFotos +
        '/" — usando cartões de exemplo só para mostrar o movimento.');
      return gerarPlaceholders(24);
    }).catch(function () { return gerarPlaceholders(24); });
  }
  ...
  function comecar() {
    ...
    /* O texto entra no FIM da animação: quando o coração se abre e as fotos
       começam a se dispersar. Para ele aparecer logo no início, defina
       CFG.atrasoTexto com um número (ex.: 300). */
    E.tTexto = {
      ini: (CFG.atrasoTexto === null || CFG.atrasoTexto === undefined)
        ? Math.max(E.t0 + 300, E.tCoracaoFim - 150)
        : E.t0 + CFG.atrasoTexto,
      dur: 1100
    };
  ...
  function iniciar() {
    ...
    descobrirFotos().then(function (urls) {
      if (!E.raiz) return;
      urlsIniciais = urls;   /* nunca vem vazio: se a pasta estiver vazia, são cartões de exemplo */
      criarFotos(urls);
      medir();
      comecar();
      vigiarDia();
  ...
  function quandoComecar(cb) {
    /* A animação NUNCA aparece na tela de senha nem na introdução:
       ela só começa quando o conteúdo principal do site fica visível.
       (o modo de teste antecipa a DATA, nunca a hora de aparecer) */
    if (CFG.iniciarQuando === "load") {
